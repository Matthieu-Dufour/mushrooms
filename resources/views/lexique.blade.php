    ...
    <div class="row">
        ...
        <div class="container collapse" id="{{ $mot->mot }}">
            <div class="row">
                @auth
                @if(Auth::user()->hasRole("mycologist"))
                <div class="col-2"><a class="btn btn-secondary welcomeBtn" href='{{route("GETeditLexique", ["id" => $mot->id])}}'>Editer</a><a class="btn btn-danger welcomeBtn" href='{{route("GETdeleteLexique", ["id" => $mot->id])}}'>Supprimer</a></div>
                @endif
                @endauth
